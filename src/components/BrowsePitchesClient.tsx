...
interface BrowsePitchesClientProps {
  posts: PitchData[];
}

const BrowsePitchesClient = ({ posts }: BrowsePitchesClientProps) => {
  ...
  const allFilteredPitches = useMemo(() => {
    const filtered = posts.filter((pitch: PitchData) => {
      // Search filter
      if (filters.search) {
...
export default BrowsePitchesClient;
